...
# Installs from GitHub, generates all 7 manuscript figures.
#
# Run anywhere: source("ggmultiplot_demo.R")
# Output: figures/Figure1.pdf ... Figure7.pdf
# ============================================================
...
cat("Installing ggmultiplot from GitHub...\n")
...
cat("[2/7] Prism bar chart + axis offset\n")
df_bar <- data.frame(
  group = c("Control","Treatment A","Treatment B"),
  mean  = c(10, 14, 8), sd = c(2, 3, 1.5)
)
...
# ---- Figure 3: Prism scatter with shape + colour mapping ----
cat("[3/7] Prism scatter with shape mapping\n")
set.seed(123)
...
# ---- Figure 4: Prism violin + boxplot overlay ----
...
fig4 <- ggplot(iris, aes(Species, Sepal.Width)) +
  ...
  ggchoice("prism") + theme(legend.position = "none") +
...
# ---- Figure 5: SPSS line chart with point shapes ----
...
fig5 <- ggplot(time_df, aes(time, value)) +
  ...
  ggchoice("spss") + scale_shape_spss() +
...
# ---- Figure 7: Scale override + axis offset ----
...
cat("Quick start: library(ggmultiplot); ggplot(data, aes(x,y)) + geom_boxplot_prism() + ggchoice('prism')\n")
