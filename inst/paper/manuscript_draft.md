# multiplot: A Unified Framework and R Implementation for Reproducing Biomedical Statistical Plot Styles Across Nine Graphing Software Packages and an Academic Publication Style
...
compliance assessment of ten supported default palettes across colour vision
...
ggplot2 (>=3.4.0), and is designed to pass R CMD check with zero errors or warnings.
...
provide (a) multi-style coverage, (b) a formal framework for reasoning about
...
2. A single-function entry point, `ggchoice("style")`, that applies the complete
   style (theme + colour scale + fill scale) of nine statistical graphing
   software-derived styles plus one academic publication style;
3. A systematic **compliance assessment** of each supported style's default palette
...
We selected nine graphing software packages commonly used in biomedical
research and added one academic publication-style benchmark [1,2]:
...
5. Academic (generic journal submission style; not a standalone software package)
...
For each software-derived style, we extracted the default visual parameters from:
...
to 29 exported functions. The package has zero compiled code and imports only
ggplot2. The `stat_compare_means_prism()` function soft-depends on ggpubr
(loaded on first use).

### 2.4 Compliance Assessment Methodology

We assessed each supported style's default discrete palette across three dimensions:
...
Key findings from the cross-style comparison:

- **Grid usage**: 7/10 supported styles do not show major grids (Prism,
  OriginPro, JMP, MATLAB, SigmaPlot, Minitab, Academic); 3/10 use light grids
  (SPSS, Stata, MedCalc).
...
The formalisation reveals that no two supported styles share an identical
5-tuple, confirming that each style's "look" is a genuinely distinctive
...
(deuteranopia/protanopia), affecting 8/10 supported default palettes. Only
...
1. **Fidelity**: Theme parameters were verified against real screenshots for
   Prism, SPSS, and MATLAB (3/10 software-derived styles), with three discrepancies corrected
   (Prism tick direction outward, SPSS light grey grid present, MATLAB outer
   box present). The remaining styles rely on documentation, scheme
...
See `inst/paper/table1_style_ontology.md` for the complete 10-style x
...
(`inst/examples/multiplot_demo.R`, included in the package repository):

**Figure S1.** 10-style comparison — the same boxplot (mpg dataset: highway
fuel economy by vehicle class) rendered in all 10 supported styles, illustrating
the visual diversity across software-derived and academic defaults.

**Figure S2.** Prism-style bar chart with T-bar error bars (`geom_col_prism()`
+ `geom_errorbar_prism()`), demonstrating the characteristic Prism column
appearance (solid fill, thin black border, outward ticks, sans-serif font).

**Figure S3.** SPSS-style scatter plot (mtcars) with discrete colour mapping.

**Figure S4.** MATLAB parula heatmap and Academic grayscale heatmap side by
side, demonstrating continuous colour scale functionality.

**Figure S5.** OriginPro-style density distribution plot with semi-transparent
fills.

**Figure S6.** Minitab-style faceted scatter plot demonstrating multi-panel
compatibility.

**Figure S7.** Scale override demonstration: Prism theme with user-supplied
ColorBrewer Set2 palette, showing the "later-wins" scale composability.

---

## Appendix B: Target Journals for Submission

Based on the scope of this work (software tool + methodology + benchmark),
the following journals are recommended in order of preference:

### 1. F1000Research — Software Tool Article (Recommended)
- **Type**: Open peer review, post-publication
- **Strengths**: Accepts GitHub-pre-published software; fast publication
  (~2-4 weeks after submission); indexed in PubMed, Scopus
- **Article length**: 1,000-3,000 words (current draft fits well)
- **Requirements**: Software must be open-source, documented, installable
- **URL**: https://f1000research.com/for-authors/article-guidelines/software-tool-articles

### 2. BMC Bioinformatics — Application Note
- **Type**: Traditional peer review
- **Impact Factor**: ~3.0 (2024)
- **Strengths**: Strong biomedical/bioinformatics readership; well-established
- **Article length**: Typically ~2,000 words + 1-2 figures
- **Requirements**: Novel computational method or tool; must have clear
  biomedical application

### 3. Journal of Open Source Software (JOSS)
- **Type**: Lightweight peer review (~2-page paper)
- **Strengths**: Minimal writing burden; fast review; well-respected in
  open-source community
- **Requirements**: Software must be installable, documented, tested;
  no new scientific contribution required beyond useful software
- **URL**: https://joss.theoj.org/

### 4. PLoS ONE
- **Type**: Traditional peer review
- **Impact Factor**: ~3.7 (2024)
- **Strengths**: Broad scope; accepts software/methodology papers
- **Requirements**: Rigorous methodology; data and code availability

### 5. Bioinformatics (Oxford) — Application Note
- **Type**: Competitive, traditional peer review
- **Impact Factor**: ~6.9 (2024)
- **Strengths**: Highest visibility in bioinformatics community
- **Requirements**: Strong biological use case demonstration; concise
  format (~2 pages printed)
- **Note**: Most competitive; would need stronger domain-specific
  application example

### Recommendation

For initial submission, **F1000Research Software Tool Article** offers the
best balance of acceptance probability, publication speed, and indexing
coverage, while accommodating the current paper structure (ontology +
compliance + software). If targeting a higher-impact venue,
**BMC Bioinformatics** is the next step, requiring a more focused
