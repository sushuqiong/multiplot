...
children.push(new Paragraph({
  ...
  children: [new TextRun({ text: "multiplot: A Unified Framework and R Implementation for Reproducing Biomedical Statistical Plot Styles Across Nine Graphing Software Packages and an Academic Publication Style", bold: true, font: "Arial", size: 36 })]
}));
children.push(new Paragraph({
  alignment: AlignmentType.CENTER, spacing: { after: 200 },
  children: [new TextRun({ text: "Shuqiong Su, Aiqun Liu*", font: "Arial", size: 28 })]
}));
children.push(new Paragraph({
...
children.push(new Paragraph({
  alignment: AlignmentType.CENTER, spacing: { after: 100 },
  children: [new TextRun({ text: "R package: multiplot v0.3.2", font: "Arial", size: 22, italics: true })]
}));
children.push(new Paragraph({
  alignment: AlignmentType.CENTER, spacing: { after: 100 },
...
children.push(boldPara("Results:", "We present (1) a formal Plot Style Ontology that decomposes a supported plot style into a 5-tuple of {Theme, Palette, Geom, Annotation, Convention} dimensions; (2) a systematic compliance assessment of ten supported default palettes across colour vision deficiency (CVD) safety, grayscale print fidelity, and CMYK gamut compatibility; and (3) multiplot, an open-source R package implementing the ontology as a ggplot2 extension. A single ggchoice(\"prism\") call applies the complete visual style of the target software. The package exports 29 functions and requires ggplot2 (>=3.4.0)."));
children.push(boldPara("Conclusions:", "multiplot fills a gap between general-purpose ggplot2 theme packages and single-software emulators, providing a unified framework for software-derived and academic plot style reproduction with built-in publication compliance guidance. The formal ontology and compliance benchmark are reusable beyond the R ecosystem."));
...
children.push(para("2. A single-function entry point, ggchoice(\"style\"), that applies the complete style (theme + colour scale + fill scale) of nine statistical graphing software-derived styles plus one academic publication style;"));
children.push(para("3. A systematic compliance assessment of each supported style's default palette against CVD safety, grayscale print fidelity, and CMYK gamut compatibility, providing actionable guidance for journal submission."));
children.push(new Paragraph({ children: [new PageBreak()] }));
...
children.push(para("We selected nine graphing software packages commonly used in biomedical research and added one academic publication-style benchmark [1,2]:"));
children.push(para("1. GraphPad Prism (v8+); 2. IBM SPSS Statistics (v12-24 classic default); 3. OriginPro (classic default; OriginLab); 4. Stata (s2color scheme, factory default pre-v18); 5. Academic (generic journal submission style; not a standalone software package); 6. SigmaPlot (v8.0+; Systat Software); 7. JMP (v13+; SAS Institute); 8. MATLAB (R2014b+ default; MathWorks); 9. Minitab (v17+; Minitab LLC); 10. MedCalc (v20+; MedCalc Software)."));
children.push(para("For each software-derived style, we extracted the default visual parameters from: (i) official software documentation and user guides; (ii) default scheme/theme definition files (e.g., Stata's scheme-s2color.scheme, MATLAB's ColorOrder defaults); (iii) direct software output inspection where available; and (iv) published descriptions in the methodology literature [3,4,11]. The Academic style is an internally defined publication-oriented benchmark. The full ontology mapping is provided in Table 1 (Supplementary Materials)."));
...
children.push(para("The package keeps ggchoice() as the primary user-facing entry point while exporting convenience scales and geoms for advanced users. It exports 29 functions and has zero compiled code."));

children.push(heading2("2.4 Compliance Assessment Methodology"));
children.push(para("We assessed each supported style's default discrete palette across three dimensions:"));
...
const cmpRows = [
  ["Multi-style coverage", "10", "1 (Prism)", "1 (SPSS)", "~8 themes*", "1 (generic)"],
...
children.push(new Table({
  ...
  rows: [
    ...
    ...cmpRows.map(row => new TableRow({ children: row.map((c, i) => cell(c, { width: [2000, 1400, 1200, 1200, 1400, 1200][i], bold: row[0] === "Multi-style coverage" || row[0] === "Formal style ontology" || row[0] === "Compliance assessment" ? (i === 1) : false })) }))
  ]
}));
...
children.push(para("The complete ontology mapping is presented in Table 1 (Supplementary Materials). Key findings from the cross-style comparison:"));
children.push(para("• Grid usage: 7/10 supported styles do not show major grids; 3/10 use light grids."));
children.push(para("• Frame: 6/10 use a full box border; 4/10 use axes-only (Academic, JMP, MATLAB, Stata)."));
children.push(para("• Font: 10/10 use sans-serif chart text in the implemented default style."));
...
children.push(para("The formalisation reveals that no two supported styles share an identical 5-tuple, confirming that each style's \"look\" is a genuinely distinctive combination rather than minor variation around a common template."));
...
const scoreHeader = ["Style", "CVD", "Grayscale", "CMYK", "Overall"];
const scoreRows = [
...
children.push(para("Package: multiplot v0.3.2"));
...
children.push(para("The complete 10-style attribute mapping across all five ontology dimensions (Theme, Palette, Geom, Annotation, Convention) is provided in the file inst/paper/table1_style_ontology.md within the multiplot package repository."));
