...
// ---- USE CASES ----
C.push(H1("Use Cases"));
C.push(body("Figures 1 through 7 illustrate the range of styles and plot types supported by ggmultiplot. All examples use only built-in R datasets (mpg, mtcars, iris) and require no external data download. Beyond what is shown, ggmultiplot provides per-software point shape scales (scale_shape_xxx()) that apply each tool's default marker sequence, and an axis_offset parameter within ggchoice() that creates the axis separation gap characteristic of Prism and SPSS output."));
...
C.push(FIGCAP("Figure 2. Prism-style bar chart with T-bar error bars and axis offset. Bars use geom_col_prism() (solid fill, thin black border); error bars use geom_errorbar_prism() (T-shaped caps wider than default). ggchoice(\"prism\", axis_offset = TRUE) creates the characteristic Prism axis separation gap at the origin."));
C.push(FIGCAP("Figure 3. Prism-style scatter plot with shape mapping (iris dataset). Points use geom_point() with colour, fill, and shape aesthetics mapped to species. scale_shape_prism() applies the Prism default shape sequence (filled circle, square, triangle)."));
...
C.push(FIGCAP("Figure 6. Continuous colour scales for heatmap data. Panel A uses ggchoice(\"matlab\") + scale_fill_matlab_c() approximating the parula colormap. Panel B uses ggchoice(\"academic\") + scale_fill_academic_c() for a CVD-safe, grayscale-compatible grayscale gradient."));
