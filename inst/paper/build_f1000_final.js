...
C.push(body("The package exports 29 user-facing functions, is implemented entirely in R, and uses ggplot2 as its only required package for core plotting functionality. Three of the nine software-derived styles (Prism, SPSS, MATLAB) were verified against real software screenshots, identifying three corrections: Prism axis ticks point outward, SPSS includes a light grey background grid, and MATLAB draws a full outer box frame. The remaining software-derived styles were checked against official documentation, user guides, or scheme files where available, while the Academic style was treated as an internally defined publication-oriented benchmark. The compliance assessment showed that the grayscale-oriented Academic and SigmaPlot styles achieved the maximum scores in this benchmark across all three assessed dimensions; the most common vulnerability was reduced colour separability under simulated colour vision deficiency, including red-green confusion in several colour-based palettes."));
...
C.push(FIGCAP("Figure 10. SPSS-style Kaplan-Meier survival curve. The lung cancer dataset (survival::lung) stratified by sex (male vs female). ggchoice(\"spss\") provides the muted blue-dominant palette, light grey grid, and centred bold title matching SPSS clinical survival output. This demonstrates multiplot on a commonly used biomedical survival-analysis example dataset."));
C.push(BR());
...
C.push(body("The Plot Style Ontology serves dual purposes. As a design document, it told us exactly which parameters to extract from each software or benchmark style. As a pedagogical tool, it gives researchers a vocabulary for describing why a Prism plot looks different from an SPSS plot—and which specific dimensions differ. Several distinctions are particularly instructive. Stata's s2color scheme is the only style with a tinted background (light bluish #F0F0F0) and horizontal-only grid lines, a convention rooted in econometric data visualisation. JMP and Academic share the absence of an outer box frame, but for different reasons: JMP reflects an interactive-exploration philosophy where visual clutter is minimised, whereas Academic reflects journal guidelines that mandate clean, ink-efficient figures. Minitab's grey background (#F0F0F0) and dark blue frame (#1F497D) derive from its quality-control heritage, where the coloured border distinguished graph output from tabular data on printed reports. MedCalc's legend-inside-plot convention is specific to clinical diagnostic accuracy testing, where placing the AUC annotation inside the plotting area maximises information density. OriginPro's classic colour order (Black, Red, Green, Blue) and outward ticks predate modern colour-science guidance and reflect the software's origins in 1990s experimental physics. SigmaPlot's single-series B&W default was designed for direct journal submission—a philosophy that, while visually conservative, yields the best print compliance in our benchmark. The fact that no two supported styles share an identical combination of the theme-level attributes in Table 1 supports the view that these are distinctive visual identities rather than minor permutations of a single default theme."));
...
C.push(body("Zenodo: multiplot — Software archive and extended data for manuscript submission. The v0.3.2 archive is available at https://doi.org/10.5281/zenodo.21239178, with the all-version concept DOI https://doi.org/10.5281/zenodo.21137144. The Zenodo archive contains the following extended data files under the inst/paper/ directory: (1) table1_style_ontology.md — complete 10-style × 15-attribute ontology mapping across all five dimensions. (2) table2_compliance_assessment.md — full CVD safety, grayscale fidelity, and CMYK compatibility evaluation with CIELAB L* values and per-colour gamut analysis. (3) verification_log.md — evidence tracking that distinguishes real screenshot checks, template screenshot checks, documentation-inferred settings, and the internally defined Academic benchmark. Figure files are provided separately as PNG and PDF outputs generated by inst/examples/multiplot_demo.R. These files are also browsable directly from the GitHub repository. Extended data are available under the terms of the Creative Commons Attribution 4.0 International license (CC-BY 4.0)."));
C.push(body("Software availability",{sz:22}));
C.push(body("Software available from: https://github.com/sushuqiong/multiplot"));
C.push(body("Archived source code at time of publication: https://doi.org/10.5281/zenodo.21239178"));
...
C.push(body("Shuqiong Su: Conceptualization, Software, Validation, Visualization, Writing – Original Draft, and Writing – Review & Editing. Aiqun Liu: Supervision, Funding Acquisition, Clinical/Application Framing, and Writing – Review & Editing."));
C.push(PL());
...
const outPath = "C:/Users/fengq/Desktop/manuscript_of_multiplot_for_F1000Research_v0.3.2_draft.docx";
